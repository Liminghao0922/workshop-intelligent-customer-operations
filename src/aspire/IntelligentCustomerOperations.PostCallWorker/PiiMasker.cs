...
public sealed record CallTurn(string Speaker, string Text, string At);
...
public static class PiiMasker
{
    ...
    {
        var turns = ExtractTurns(transcriptNode);
        var azureRedaction = await TryAzureLanguageRedactionAsync(turns, language, ct);
        if (azureRedaction is not null)
        {
            return azureRedaction;
        }
    ...
    private static IReadOnlyList<CallTurn> ExtractTurns(JsonNode? transcriptNode)
    {
        var turns = new List<CallTurn>();

        if (transcriptNode is JsonArray transcript)
        {
            foreach (var entry in transcript)
            {
                if (entry is JsonObject turn)
                {
                    turns.Add(
                        new CallTurn(
                            turn["speaker"]?.GetValue<string>() ?? "unknown",
                            turn["text"]?.GetValue<string>() ?? string.Empty,
                            turn["at"]?.GetValue<string>() ?? string.Empty
                        )
                    );
                }
            }
    ...
    private static PiiRedactionResult MaskTranscriptLocally(IReadOnlyList<CallTurn> turns)
    {
